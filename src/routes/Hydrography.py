...
@main.route('/LEM/historical', methods=['GET'])
def get_historial():
    
    try:
        station_id = request.json['id']
        station_name = request.json['Nombre']

        response = HydrographyModel().get_historial(station_id, station_name)
        print(str(response.index))
...
@main.route('/LEM/observado', methods=['GET'])
def get_observado():
...
@main.route('/LEM/forecast', methods=['GET'])
def get_furecast():
...
@main.route('/LEM/run', methods=['GET'])
def API_LEM():
    
    try:
        
        section = request.json['section']
        initial_date = request.json['initial date']
        final_date = request.json['final date']
        subcatchment_id = request.json['subcatchment id']
        Nombre = request.json['Nombre']

        inputs = {'section':section,
                  'initial date': initial_date,
                  'final date': final_date}
        param = {"id":subcatchment_id, "Nombre": Nombre}
        response = LEM().LEM_simulation(inputs,param)
        
        if response:
            return jsonify(response)
        else:
            return jsonify({'message': "Error on id or name"}), 500
